...
{#snippet sec(key)}
  ...
  {:else if key === 'education' && data.education.length}
    ...
    {#each data.education as e}
      <div class="mini"><strong>{e.degree}</strong><span>{e.school}</span><em>{dates(e.start, e.end)}{e.note ? ' · ' + e.note : ''}</em>{#if e.skills?.length}<div class="entry-skills">{#each e.skills as s}<span>{s}</span>{/each}</div>{/if}</div>
    {/each}
